...
func (s *server) handleSend(w http.ResponseWriter, r *http.Request) {
    ...
    // Respond immediately to avoid upstream timeouts; send in background
    accepted := len(req.Tokens)
    ...
    // Background send with independent timeout per batch
    go func(req requestPayload) {
        ...
        log.Printf("push-service: sent=%d/%d", sent, accepted)
    }(req)
}
