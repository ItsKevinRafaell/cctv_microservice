...
def main():
    ...
    # 4) Create camera
    r = requests.post(f"{API_MAIN}/api/cameras", headers=headers, json={"name": "Lobby", "location": "L1"})
    ok = r.status_code in (200,201)
    ...
    if fcm_token:
        ...
        _print("Update FCM token", True, "skipped (no FCM_TOKEN env)")
